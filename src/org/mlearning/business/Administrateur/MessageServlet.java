...
public class MessageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if( request.getSession().getAttribute("loggedUser")==null ) response.sendRedirect("InscriptionServlet?code=-1");
			...
			User u = (User)session.get(User.class, loggedUser.getId());
			request.setAttribute("listeDesMessages", u.getMessage());
			
			request.getRequestDispatcher("Administrateur/messages.jsp").forward(request, response);
		}
	}
