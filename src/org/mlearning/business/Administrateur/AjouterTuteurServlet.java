 ...
 */
@WebServlet("/AjouterTuteurServlet")
public class AjouterTuteurServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if( request.getSession().getAttribute("loggedUser")==null ) response.sendRedirect("InscriptionServlet?code=-1");
			...
			request.getRequestDispatcher("Administrateur/ajouterTuteur.jsp").forward(request, response);
		}
	}
			...
			try{
				...
				while ( i.hasNext () ) {
					...
					if(pass){
						...
						t.setNom(nom);
						t.setLogin(login);
						t.setPassword( User.md5(password) );
						t.setIdentifiantunique(identifiant);
						t.setNatureidentifiant(natureidentifiant);
						t.setEmail(email);
						t.setPhone(phone);
						t.setPays(pays);
						t.setVille(ville);
						t.setAdresse(adresse);
						t.setNaissance(naissance);
						t.setDateinscription(dateinscription);
						t.setPhoto(fileName);
						t.setIsactive(false);
						
						session.saveOrUpdate(t);
						session.getTransaction().commit();
						response.sendRedirect("TuteursServlet?code=1");
					}	
				}
				}catch(Exception ex) {
					out.println(ex);
