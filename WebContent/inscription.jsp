...
<link rel="stylesheet" type="text/css" href="css/administrateur.css" />
	<div class="mainwrapper">
		<div class="rightpanel">
			...
			<div class="pagetitle">
				<h1>INSCRIPTION</h1> <span>Vous êtes la bienvenue...</span>
			</div>
						...
						<% if( !(request.getParameter("code")).equals("null")){ %>
							<% if( (request.getParameter("code")).equals("1")){ %>
								<div class="alert alert-success">
									...
									<strong>Votre demande d'inscriptiona été bien envoyé</strong>
								</div>
							<% } %>
							<% if( (request.getParameter("code")).equals("2")){ %>
								<div class="alert alert-warning">
									<button data-dismiss="alert" class="close" type="button">x</button>
									<strong>Code de vérification anti-spam est incorrect!</strong>
								</div>
							<% } %>							
							<% if( (request.getParameter("code")).equals("-1")){ %>
								<div class="alert alert-error">
									<button data-dismiss="alert" class="close" type="button">x</button>
									<strong>Cette plateforme est sécurisée!</strong><br/>
									L'accès est seulement autorisé pour les utilisateurs déjà inscrits.
									<ul>
										<li>Cliquer <a href="index.jsp">ici</a> pour se connecter.</li>
										<li>Vous pouvez utiliser ce formulaire pour demander une inscription.</li>
										<li>Pour plus d'information, veuillez nous sur le num (+41) 022 320 31 77.</li>
									</ul>
								</div>
							<% } %>	
						<% } %>
					<h2 align="center" class="d_title">Demande d'inscription</h2><hr/>
					<form action="InscriptionServlet" method="POST" class="stdform" id="ajouterApprenantForm" novalidate="novalidate" enctype="multipart/form-data">
						...
						<div class="par control-group">
							<label for="recaptcha" class="control-label">Code Anti-spam</label>
							<div class="controls">
								<%
									ReCaptcha c = ReCaptchaFactory.newReCaptcha("6LeMx-cSAAAAALKD0s95jz0ASpfxbEzKcvgj_A1U", "6LeMx-cSAAAAAGdRe5H_B2UefZBun9LONZlNQ-MN", false);
									out.print(c.createRecaptchaHtml(null, null));
								%>
							</div>	
						</div>						
						<p class="stdformbutton">
							<button class="btn btn-primary btn-rounded">Envoyer demande</button>
						</p>
					</form>
				</div>
			</div>	</p>
